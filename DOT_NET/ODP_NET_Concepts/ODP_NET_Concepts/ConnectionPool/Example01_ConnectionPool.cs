...
namespace ODP_NET_Concepts.ConnectionPool
{
	class Example01_ConnectionPool
	{
			...
			using (IDbConnection connection = ConnectionPool.ConnectionUtil_Pooling.GetConnection())
			{
				using (IDbCommand command = connection.CreateCommand())
				{
					try
					{
						...
						command.CommandText = "select mbr, ime, prz, plt from radnik where ime like :ime and prz like :prz";    //:ime i :prz su bind promenljive;
																																//primetiti da se promenljive pisu bez
																																//navodnika iako ce imati varchar2 vrednosti
						command.Parameters.Add(new OracleParameter("ime", OracleDbType.Varchar2, 25));              //imena parametara ne zavise od imena bind
						command.Parameters.Add(new OracleParameter("prezime", OracleDbType.Varchar2, 25));          //promenljive, samo je bitan redosled
						command.Prepare();
		...
		private static void SelectRadnikByImeAndPrz(IDbCommand preparedCommand)
		{
			...
			ParameterUtil.SetParameterValue(preparedCommand, "ime", Console.ReadLine());    // parametre nalazimo po imenu koje smo dali pri kreiranju istih
