...
{
    public static class Example02_ManualCommit
    {
		public static void Example()
		{
			...
			ExecuteTransactional("update radproj2 set brc = brc * 0.5",
								 "update radnik2 set plt = plt * 0.5");
		...
		private static void ExecuteTransactional(params string[] sqls)
		{
				...
				try
				{
					...
					using (IDbCommand command = connection.CreateCommand())
					{
						...
						catch (DbException ex)
						{
							...
							transaction.Rollback(); // if one of the commands fails, roll-back changes
						...
						using (IDataReader reader = command.ExecuteReader())
						{
							...
							while (reader.Read())
							{
								...
								Console.WriteLine("{0,-4} {1,-8} {2,-9} {3,8:F2} {4,3} {5,3}",
									reader.GetInt32(0), reader.GetString(1), reader.GetString(2), reader.GetFloat(3),
