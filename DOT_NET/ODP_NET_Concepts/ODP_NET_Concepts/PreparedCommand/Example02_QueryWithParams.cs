			...
			using (IDbConnection connection = Connection.ConnectionUtil_Basic.GetConnection())
			{
				try
				{
					...
					using (IDbCommand command = connection.CreateCommand())
					{
						...
						ParameterUtil.AddParameter(command, "ime",		DbType.String, 25);		//imena parametara ne zavise od imena bind
						ParameterUtil.AddParameter(command, "prezime",	DbType.String, 25);		//promenljive, samo je bitan redosled
						command.Prepare();
