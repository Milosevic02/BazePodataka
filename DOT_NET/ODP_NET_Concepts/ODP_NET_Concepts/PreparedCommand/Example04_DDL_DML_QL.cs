    ...
    public static class Example04_DDL_DML_QL
    {
		...
		private static void HandleMenu()
		{
			...
			do
			{
                ...
                Console.WriteLine("7 - Radnici with more brc");
		...
		{
			string sql = "create table faze_projekta (spr int, sfp int, rukfp int, nafp varchar2(10), datp date,"
		...
		private static void radniciBrc()
		{
			Console.WriteLine("SPR: ");
			string spr = Console.ReadLine();
			Console.WriteLine("BRC: ");
			int brc = Int32.Parse(Console.ReadLine());
			...
			using(IDbConnection connection = Connection.ConnectionUtil_Basic.GetConnection())
			{
				try
				{
					...
					using(IDbCommand command = connection.CreateCommand())
					{
						...
						command.CommandText = "select count(mbr) from radproj where spr = :spr and brc > :brc";
						ParameterUtil.AddParameter(command, "spr", DbType.Int32, 0);
						ParameterUtil.AddParameter(command, "brc", DbType.Int32, 0);
						command.Prepare();
						...
						using (IDataReader reader = command.ExecuteReader())
						{
							Console.WriteLine("{0,-4}", "BROJ");
							if (((DbDataReader)reader).HasRows)
							{
								while (reader.Read())
								{
									Console.WriteLine("{0,-4}", reader.IsDBNull(0) ? "" : reader.GetString(0));
								}
							}
							else
							{
		...
		private static void InsertRow()
		{
			...
			string sql = "insert into faze_projekta (spr, sfp, rukfp, nafp, datp) values (:spr, :sfp, :rukfp, :nafp, to_date(:datp, 'dd.MM.yyyy.'))";
			...
			using (IDbConnection connection = Connection.ConnectionUtil_Basic.GetConnection())
			{
				try
				{
					...
					using (IDbCommand command = connection.CreateCommand())
					{
						...
						ParameterUtil.AddParameter(command, "datp", DbType.String, 10); //to_date funkcija prima string kao parametar
						command.Prepare();
		...
		private static void ModifyRow()
		{
			...
			string sql = "update faze_projekta set rukfp=:rukfp, nafp=:nafp, datp=to_date(:datp, 'dd.MM.yyyy.') where spr=:spr and sfp=:sfp";
			...
			using (IDbConnection connection = Connection.ConnectionUtil_Basic.GetConnection())
			{
				try {
					...
					{
						command.CommandText = sql;
						ParameterUtil.AddParameter(command, "rukfp", DbType.Int32, 0);  // obratiti pažnju na redosled parametara
						ParameterUtil.AddParameter(command, "nafp", DbType.String, 20);
