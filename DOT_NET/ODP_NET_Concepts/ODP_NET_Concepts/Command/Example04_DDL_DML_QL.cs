    ...
    public class Example04_DDL_DML_QL
    {
        ...
        private static void HandleMenu()
        {
			...
			do
			{
				...
				Console.WriteLine("7 - Select radnik working in projects with spr = 10 and not working in project with spr=30");
        ...
        private static void Radnik10not30()
        {
			string sql = "select r.mbr,ime,prz from radnik r ,radproj rp where r.mbr = rp.mbr and rp.spr = 10 and rp.mbr not in (select mbr from radproj where spr = 30)";
			...
			using(IDbConnection connection = Connection.ConnectionUtil_Basic.GetConnection())
			{
				try
				{
					...
					using(IDbCommand command = connection.CreateCommand())
					{
						...
						{
							Console.WriteLine("{0,-4} {1,-8} {1,-8}", "MBR", "IME", "PRZ");
		...
		private static void InsertRow()
		{
			...
			string sql = string.Format(
					"insert into faze_projekta (spr, sfp, rukfp, nafp, datp) values ({0}, {1}, {2}, '{3}', to_date({4}, 'dd.MM.yyyy.'))",
		...
		private static void DeleteRow()
		{
			...
			string sql = string.Format("delete from faze_projekta where spr={0} and sfp={1}",
					string.IsNullOrEmpty(spr) ? "null" : spr,
					string.IsNullOrEmpty(sfp) ? "null" : sfp);
